...
class InputValidator:
    ...
    def sanitize_string(cls, input_str: str, max_length: int = 1000) -> str:
        ...
        sanitized = ''.join(c for c in input_str if ord(c)
                            >= 32 or c in '\t\n\r')
        ...
        for path_key in required_paths:
            ...
            if isinstance(path_value, str):
                if not cls.validate_file_path(path_value):
                    logger.error(
                        f"Invalid path format: {path_key}="
                        f"{path_value}")
                    return False
        ...
        for timing_key in timing_keys:
            if timing_key in timing:
                ...
                if not isinstance(value, (int, float)) or value <= 0:
                    logger.error(
                        f"Invalid timing value: {timing_key}={value}")
                    return False

        return True

    @classmethod
    def validate_ignore_list(cls, ignore_list: List[str],
                             list_type: str) -> List[str]:
        """Validate and filter ignore list entries"""
        if not isinstance(ignore_list, list):
            logger.error(
                f"Ignore list must be a list, got {type(ignore_list)}")
            return []

        validated_list = []
        validator_func = (cls.validate_mac_address if list_type == 'mac'
                          else cls.validate_ssid)
    ...
    @classmethod
    def validate_json_input(cls, json_str: str,
                            max_size: int = 1024*1024) -> Optional[Dict]:
                ...
                for key, value in data.items():
                    ...
                    if isinstance(value, (dict, list)):
                        ...
                        if len(value) > 10000:  # Reasonable string limit
                            logger.warning(
                                f"JSON string value too long: {key}")
                            return None
    ...
    def validate_database_path(cls, db_path: str) -> bool:
        ...
        if '*' in db_path:
            ...
            if base_dir and not Path(base_dir).exists():
                logger.warning(
                    f"Database base directory does not exist: {base_dir}")
                return False
        else:
