...
class SecureCredentialManager:
    ...
    def _get_master_password(self) -> str:
        ...
        try:
            password = getpass.getpass(
                "Enter master password for CYT credentials: ")
            if not password:
            ...
            print(
                "Set CYT_MASTER_PASSWORD environment variable or use "
                "CYT_TEST_MODE=true for testing")
            raise RuntimeError(
                "Password entry not available in non-interactive mode")
    ...
    def store_credential(self, service: str, credential_type: str,
        ...
        if not all(isinstance(x, str)
                   for x in [service, credential_type, value]):
    ...
    def migrate_from_config(self, config: Dict[str, Any]) -> None:
        ...
        if 'encoded_token' in wigle_config:
            ...
            print("Found WiGLE API token in config file - "
...
def secure_config_loader(config_path: str = 'config.json') -> Dict[str, Any]:
    ...
    if 'api_keys' in config:
        ...
        if any('token' in str(api_keys).lower() or 'key' in str(
                api_keys).lower() for key in api_keys.values()
                if isinstance(key, dict)):

            logger.warning(
                "Found API keys in config file - "
                "initiating secure migration")
            cred_manager.migrate_from_config(config)

            config.pop('api_keys', None)

            sanitized_config_path = config_path.replace(
                '.json', '_sanitized.json')
            with open(sanitized_config_path, 'w') as f:
                json.dump(config, f, indent=2)

            print(
                f"✅ Created sanitized config file: {sanitized_config_path}")
            print(
                "⚠️  Please replace your config.json with the "
                "sanitized version")
