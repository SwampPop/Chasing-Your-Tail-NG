...
class HomeMonitor:
            ...
            for dev in devices:
                ...
                if is_night_mode:
                    ...
                    self.alerter.send_alert(
                        f"🌙 NIGHT ALERT: New Device {mac} detected at "
                        f"{current_hour}:00")
                    self.night_threats.append(mac)

                # 4. Auto-Learn (Optional)
                # If we see a device for > 1 hour during the day,
    ...
    def generate_daily_report(self):
        ...
        sorted_devs = sorted(self.daily_stats.items(),
    ...
    def run(self):
        ...
        if not files:
            ...
            if os.path.exists("test_capture.kismet"):
                ...
                logger.error(
                    f"No Kismet database files found at: {db_pattern}")
                return
        else:
