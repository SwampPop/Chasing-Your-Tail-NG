...
def main():
    ...
    # Ask user to overwrite original config
    overwrite = input(
        "Do you want to overwrite 'config.json' with the sanitized version? (y/n): ")
    if overwrite.lower() == 'y':
        os.rename(sanitized_file, 'config.json')
        print("✅ Overwrote 'config.json' with the sanitized version.")
        print("   A backup of the original was saved to 'config_backup.json'")
    else:
        print("Next steps:")
    ...
    print("\n⚠️  IMPORTANT: Your API keys are now encrypted and "
